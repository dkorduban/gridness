...
class GridnessTest {
    ...
    /**
     * Sparse fixtures (few buildings per R-window) need a low Hough peak
     * threshold and small tiles so a tile sees a meaningful slice of the
     * layout instead of one quadrant of the whole field.
     */
    private static GridnessParams sparseLayoutParams() {
        return GridnessParams.builder()
                .tileSize(32).tileStride(32).houghMinPeakWeight(5).build();
    }
